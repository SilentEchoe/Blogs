...
Routing 想将这些衍生出来的问题，通过不同路由分发，将不同问题映射到更相近的模版。这就是Routing 想做的事情。



#### **Logincal**

当一个问题产生后，需要思考的是：这个问题是否还适用于使用向量索引来进行检索，如果使用传统的SQL查询会不会更好？

使用RAG一个常见的情况是：用户输入一个指向性非常具体的问题，比如某人某年的某一个事件的细节。这种情况下不需要任何生成式的"创新"，但在向量数据库中已经存有大量的关联性数据，在这种关联数据过于紧密的情况下，反而会分散注意力，导致查询出来的效果并不理解。

在这种情况下，使用传统的图数据库可能是一种更好的选择。

路由的作用就是在不同情况下使用不同的检索数据，但是这里要注意的是：路由不是通过编程去实现的，它和传统的网络路由虽然有相识的部分，但是在AI领域，路由需要通过大语言模型的相关能力来完成。
