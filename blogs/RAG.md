...
在[优化问题]中，将一个问题变成多个问题为的是丰富上下文的信息，将多个文档聚合的每个阶段都保留下来，也是为了有更好的召回率，而且细化了每个节点，增加了命中率。
...
实际应用场景下会更复杂且多变，对于某些特定领域，使用通用的向量化可能也不太适用。在某些场景下建议使用专有的向量化模型进行enbeddings，比如ColBERT。
...
比较理想化的情况是：让各个领域训练专有模型，在此基础上构建AI应用会相对容易，也更容易发挥出模型的能力。但反过来思考，可能某些领域受限于高昂的算力和成本走向弱势化，最终趋同与单一化，这样也无法推动AGI的发展。
