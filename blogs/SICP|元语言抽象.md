...
元语言抽象(Metalinguistic Abstraction)在工程设计中扮演着重要角色。DSL(领域专用语言) 是这种能力在工程中的一种常见产物，这使工程师不仅可以使用现有的高级编程语言，还可以通过编写**解释器**来创造新语言，比如Go语言最初的解释器是C语言写的，在经历数个版本后才使用Go语言实现**自举**。
...
这一章虽然例举了大量的代码来描述“解释器”的工作原理，但是要值得特别表述的是：“**语言的语义本身可以被程序化地构造**”。当工程师用某种元语言实现一个对象语言的解释器时，解释器中的 eval/apply 规则、环境模型、特殊形式（if、define、set! 等）共同构成了这门语言“意义”的可执行定义。

**改变解释器，就是在改变语言**。

元语言抽象的价值在于：它可以把“抽象”推进到语言层级，工程上可以落地为DSL、规则引擎、查询语言、工作流语言等。

**解释器当作语义的载体，语言便可当作可重构的抽象边界**。
